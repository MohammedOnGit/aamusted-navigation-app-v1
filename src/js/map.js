...
function setupMarkerAndFly(coordinates, buttonId, speed, popupText) {
  ...
  const marker = searchMarkers.find((marker) => marker.options.title === buttonId);
...
coordinatesData.forEach(function (data) {
  setupMarkerAndFly(data.coordinates, data.buttonId, 0.5, data.popupText,searchMarkers);
});
